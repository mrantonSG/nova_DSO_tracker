...
def parse_phd2_log(content: str) -> Dict[str, Any]:
    ...
    for line in lines:
        ...
        if "Guiding Begins" in line:
            ...
            # Start new session
            current_session = {
                'start_idx': len(result['frames']),
                'frames': [],  # [(ra_px, dec_px, snr), ...]
                'start_dt': dt
            }
        ...
        if "Guiding Ends" in line:
            if current_session is not None:
                ...
                # Calculate end hours
                if current_session['frames']:
                    end_h = (current_session['start_idx'] + len(current_session['frames'])) * 1.0 / 1800  # ~2s per frame
                    result['run_bounds'].append({
                        'run': len(sessions),
                        'h': round(end_h, 4),
                        'type': 'end'
        ...
        if "SETTLING STATE CHANGE" in line.upper():
            ...
            if 'settling started' in lower_line or 'begun' in lower_line:
                # Mark start of settling (we'd need more context to track full cycle)
                pass
        ...
        if current_session is not None and line and line[0].isdigit() and ',' in line:
            ...
            if ra is not None and dec is not None:
                current_session['frames'].append((ra, dec, snr if snr else 0))
    ...
    # Use the longest session as the main one
    main_session = max(sessions, key=lambda s: len(s['frames']))

    if not main_session['frames']:
        return result

    # Build frames array with hours (assuming ~2 seconds per guide frame)
    frame_interval_hours = 2.0 / 3600.0  # 2 seconds in hours
    for i, (ra, dec, snr) in enumerate(main_session['frames']):
        hours = i * frame_interval_hours
        result['frames'].append([round(hours, 4), ra, dec, snr])

    # Compute rolling RMS (30-frame window)
    window = 30
    frames_data = main_session['frames']
    ps = result['pixel_scale']

    for i in range(window, len(frames_data)):
        window_frames = frames_data[i - window:i]
        ra_vals = [f[0] for f in window_frames]
        dec_vals = [f[1] for f in window_frames]
    ...
    all_ra = [f[0] for f in frames_data]
    all_dec = [f[1] for f in frames_data]
    ...
    result['stats']['total_frames'] = len(frames_data)

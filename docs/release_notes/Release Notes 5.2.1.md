...
- **Database migration fix (5.2.1)** - Added missing `ALTER TABLE` patches for `stable_uid` columns that were causing `OperationalError: no such column` crashes on existing deployments upgrading from pre-5.2 versions
...
- **Database migration** - Automatic on app startup (new columns for structured dither fields, custom filters, guide optics, analytics tables, and stable_uid fields)
- **Seamless upgrade** - No manual steps required for any migration
